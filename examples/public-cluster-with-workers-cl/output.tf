locals {
  ...
  ssh_prefix      = "ssh core@${module.k8s_masters.public_ipv4_addrs[0]} --"

  test_ssh_prefix = <<CMD
ssh -o UserKnownHostsFile=/dev/null \
    -o StrictHostKeyChecking=no \
    core@${module.k8s_masters.public_ipv4_addrs[0]} -- \
  CMD
}
...
${replace("${local.test_ssh_prefix} sh -c \"'${module.k8s_masters.etcd_status}'\"", "/\\s*\\\\\\s+/", " ")} \
`# This output is only for test purpose!`
CMD
}
...
Check if cluster is running:
    ...
    $ ${indent(6, local.ssh_prefix)} sh -c \"'${indent(6, module.k8s_masters.etcd_status)}'\" && echo 'etcd cluster is running'

    $ ${indent(6, local.ssh_prefix)} sh -c \"'${indent(6, module.k8s_masters.k8s_status)}'\" && echo 'k8s cluster is running'

Configure the client:

    $ ${indent(6, local.ssh_prefix)} ${indent(6, module.k8s_masters.k8s_get_config)} > ${local.k8s_config_path}
